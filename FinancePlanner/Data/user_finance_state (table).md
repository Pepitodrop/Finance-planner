...
- **Extended again 2026-08-27 (PR #154) for `Account.stableId` and `Transaction.stableTransactionId`** (reconnect-identity fields, same allow-list-extension pattern as the sixth-pass fix above) — see [[Stable Account Identity and Reconnect Reconciliation]]. **Architectural note:** the related `connector_account_exclusions` table (migration 011, also added this pass) is deliberately a SEPARATE table, not part of this encrypted finance-state blob — an account-sync exclusion is a connector-sync preference tied to a provider connection, not financial data, so it does not go through `validateCloudPayload()`'s schema at all.

Related: [[Data Index]] · [[Data and Persistence]] · [[Vault Conflict Page]] · [[Enable Banking]] · [[Provider Status]] · [[Stable Account Identity and Reconnect Reconciliation]]
