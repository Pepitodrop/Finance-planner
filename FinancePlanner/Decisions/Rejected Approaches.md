...
**Considered, then rejected:** patching (rather than removing) `ConnectionsPage.tsx`'s `popupAttempt.popup.closed` polling to detect a manually-closed provider-authorization popup.
**Why rejected:** Finance Planner sends `Cross-Origin-Opener-Policy: same-origin` (a real cross-window-attack mitigation, kept intentionally). Once the popup navigates cross-origin to the real provider, that policy severs the opener's `WindowProxy` reference to it, so `.closed` can read `true` while the authorization window is genuinely still open — and `startConnector()` only ever returns `{mode: 'popup'}` *after* that navigation already happened, so every poll this code ever performed was already past the point the signal became unreliable. A patched heuristic (trusting `.closed` for a bounded window, or pairing it with a second confirming signal) was considered and rejected because no reliable secondary signal exists once the popup is on a cross-origin page — Finance Planner's JS has zero visibility into that page by design (see [[Provider Authorization Popup Bridge]]). Weakening COOP to restore visibility was also rejected outright, since it's a real security control, not incidental to this feature. Fixed by removing the automatic detection entirely: an always-available manual "Try again" action (not conditioned on any detected state) abandons the current popup before starting a fresh one, and a genuine completion is still driven entirely by the bounded popup-return bridge, never by `.closed`. `window.name` was also considered as an alternative context-binding mechanism for a separate, related fix (see [[Provider Authorization Popup Bridge]]'s "Popup-context binding" section) and rejected in favor of a `sessionStorage`-clone-at-creation marker, since some browsers' anti-tracking mitigations can clear `window.name` on cross-site navigation, which would silently break exactly the round trip this bridge depends on. (PR #154 review, 2026-08-25)
**Related:** [[Provider Authorization Popup Bridge]], [[Connections Page]]
