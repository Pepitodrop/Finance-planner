...
- **Account/balance/transaction sync: code-fixed, awaiting runtime re-verification** — the first sync after that persisted connection failed with a real provider HTTP 422 (root-caused: `GET /sessions/{id}.accounts` is bare account-id strings, not `AccountResource` objects; see [[Enable Banking]]'s fourth-pass entry for the full fix). A third PR #154 review round (same day) found a second, unrelated transaction-contract gap in the same `sync()` method: statuses beyond `BOOK`/`PDNG` (`CNCL`/`HOLD`/`OTHR`/`RJCT`/`SCHD`) were silently imported as booked; fixed to skip those five outright and fail closed on anything undocumented. Both fixes covered by 43 local tests against mocked responses; neither exercised against a real session yet.
...
- **No-reunlock popup return: code-fixed, awaiting runtime verification** — the provider-authorization popup bridge (`src/providerReturnBridge.ts`) was exercised for the first time this pass against a real provider callback. Three PR #154 review rounds (same day) found and fixed, in order: (1) an unreachable popup-blocked fallback and a silently-never-executing test suite; (2) that fix #1's fallback was itself wrong for production — falling through to a same-tab redirect/embedded widget on a blocked popup recreates the exact vault-reset regression this bridge exists to prevent — replaced with a genuine fail-closed rejection before `/start`, plus bounded provider/error validation on the return signal and a logout cleanup fix; (3) `Window.closed` polling was not a reliable manual-close signal once the popup navigates cross-origin under this app's `Cross-Origin-Opener-Policy: same-origin` header (removed, not patched), the pre-React bootstrap short-circuit was triggered by URL shape alone rather than being bound to the popup's own browsing context (fixed with a sessionStorage-clone-at-creation marker), and logout's browser-side cleanup only ran after a successful server response (fixed with `try/finally`). See [[Bank Connections]], [[Provider Callback Binding]], and [[Provider Authorization Popup Bridge]] for full detail.
...
No-reunlock popup return: code-fixed (three PR #154 review rounds), independently reviewed, awaiting runtime verification.
```
