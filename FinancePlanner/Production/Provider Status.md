...
- **No-reunlock popup return: code-fixed, awaiting runtime verification** — the provider-authorization popup bridge (`src/providerReturnBridge.ts`) was exercised for the first time this pass against a real provider callback. A first review pass found and fixed an unreachable popup-blocked fallback and a silently-never-executing test suite; a second review pass (PR #154) then correctly identified that the fallback fix itself was wrong for production — falling through to a same-tab redirect/embedded widget on a blocked popup recreates the exact vault-reset regression this bridge exists to prevent — and replaced it with a genuine fail-closed rejection before `/start` is ever called, plus bounded provider/error validation on the return signal and a logout cleanup fix. See [[Bank Connections]], [[Provider Callback Binding]], and [[Provider Authorization Popup Bridge]].
