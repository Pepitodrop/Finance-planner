...
### Live verification matrix (PR #144, temporary deployment against finance.luisbenedikt.de, 2026-08-21)

| Capability | Status | Evidence |
|---|---|---|
| Enable Banking configured/available | **YES** | Connections UI showed it selectable |
| `GET /aspsps` (live directory) | **YES** | Real response, 592 institutions returned for DE |
| Institution filtering/search (`directoryTerms`, blank-query family view, forgiving text search) | **YES** | "Volksbank / Raiffeisenbank" opened blank and showed real branches (Aachener Bank, Berliner Volksbank, Volksbank Köln Bonn, Volksbank Rhein-Erft-Köln, ...) with real BICs; "Berlin" and "Köln" searches worked |
| Exact-bank selection reaching Step 3 confirmation | **YES** | Selecting a real bank reached the redirect-confirmation screen |
| `POST /auth` (redirect to provider) | **NO** — failed with a generic "Internal server error" pressing "Continue securely" for Volksbank Köln Bonn. Root-caused (not confirmed via production logs — none were available to this session — but confirmed against current official Enable Banking API docs and unit-tested): `EnableBankingProvider.start()` treated `match.maximum_consent_validity` (documented as **seconds**) as if it were already **days**, so the per-ASPSP consent-duration clamp never actually fired and every request silently asked for the full default (90-day) window regardless of what the selected bank's ASPSP actually supports — a bank with a real cap under that default gets `POST /auth` rejected by Enable Banking's own documented `valid_until ≤ now + maximum_consent_validity` rule. Fixed in the same commit (seconds→ms conversion) plus `access.balances`/`transactions: true` added to match what the UI already tells the user, plus improved server-side (never client-facing) provider-error logging so a future occurrence is diagnosable from logs. **Not yet re-verified live** — this session was not permitted to redeploy. | `server/src/providers.js` `EnableBankingProvider.start()`, `server/test/enable-banking-callback.test.js` |
| Consent (user completes bank-side auth) | **NO** | Blocked by the `/auth` failure above |
...
Current limitations: same class of gap as GoCardless below — no completed end-to-end consent→sync→disconnect cycle evidenced against a live sandbox, and the one live attempt made so far failed at `POST /auth` for the reason above. Requires Control Panel setup (sandbox application, allowed callback URL `{APP_ORIGIN}/api/connectors/callback`, application id, RSA private key) — confirmed already done, since discovery/JWT auth succeeded live — before consent/sync verification can proceed. See [[Enable Banking]] and [[Bank Family Directory Resolution]].
