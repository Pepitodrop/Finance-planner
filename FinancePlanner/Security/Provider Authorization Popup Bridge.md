...
- **Popup blockers**: a blocked popup is a visible, retryable failure *before* `/start` is ever contacted and before any OAuth nonce is created — never a silent same-tab fallback (which would reintroduce the exact vault-reset regression this bridge exists to avoid).
...
`src/providerReturnBridge.test.ts` (11 cases): attempt binding, popup-blocker failure, mismatch rejection, code/state/error_description leakage prevention, single-use buffered-return consumption, expiry, replay rejection, `abandonConnectorPopupAttempt`, `clearPendingConnectorAttempt`, BroadcastChannel delivery of a valid signal, and rejection of a malformed one. `src/connectors.startConnector.test.ts` (+6 cases): popup-success mode/no-navigation, provider-agnostic behavior, the popup-specific return URL in the `/start` request body, popup abandonment on a failed `/start`, and the popup-blocked/storage-blocked fallback actually being reachable. `src/features/connections/ConnectionsPage.test.tsx` (+6 cases): busy clears and the waiting copy renders; manual popup closure is detected and offers Try again; Try again starts a fresh attempt; Cancel/Back/closing the setup dialog all genuinely close the real popup. `src/AuthGate.test.tsx` (+1 case): logout clears the pending attempt binding.

**Not yet re-verified live** — the fourth Mock ASPSP pass (2026-08-25) exercised this bridge against a real provider callback for the first time (see [[Provider Status]]), but that was before the two bugs above were found and fixed; a fresh live pass is needed to confirm the corrected behavior end to end.
