...
## Fifth pass (2026-08-25): the core popup/vault-preservation mechanism is now LIVE VERIFIED

A temporary deployment carrying every fix through the third review round above was tested against Enable Banking's real Mock ASPSP sandbox in Firefox. **Verified live**: the separate provider popup opens correctly, the original Finance Planner tab remains mounted throughout, the original vault remains unlocked throughout, and `POST /api/connectors/enablebanking/start` reaches the provider and authorization completes server-side with a connection genuinely persisted. This is the first live confirmation that the core reason this bridge exists — preventing the same-tab vault-reset regression — actually works end to end in a real browser, not just in jsdom-simulated tests.

This pass also surfaced a real, unrelated bug in the server-side callback route itself (a concurrent-duplicate-callback race, two deliveries of the same authorization producing a spurious `invalid_state` for one of them) — fixed, documented in [[Provider Callback Binding]]'s "Fixed 2026-08-25: concurrent-duplicate-callback race" section. That fix is orthogonal to this bridge (it lives entirely server-side, in `provider-callback.js`/`postgres-store.js`/`crypto-store.js`) and does not change anything documented in this note.

**Still not re-verified live**: whether a real cross-origin popup round trip actually triggers the `sessionStorage` clone the popup-context-binding fix depends on (this pass's popup never needed to be closed/reopened/retried, so the marker path specifically wasn't exercised, only the happy path was); the `Window.closed`-removal fix (no popup was manually closed during this pass to test the "Try again" UI); and whether a genuinely blocked popup or unavailable `sessionStorage` produces the expected fail-closed error text in a real browser (this pass's popup was never blocked). All are still code-fixed and test-verified only.
