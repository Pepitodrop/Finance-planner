...
const SCRIPT = resolve('scripts/finance-intelligence-production-acceptance.mjs')
...
for (attempts = 1; attempts <= MAX_TRANSIENT_ATTEMPTS; attempts += 1) {
  ...
  console.warn(`Retrying Step 12 Finance Intelligence acceptance after transient browser failure (${attempts}/${MAX_TRANSIENT_ATTEMPTS}): ${report.error}`)
  await new Promise((resolveDelay) => setTimeout(resolveDelay, 500 * attempts))
}

if (result.code !== 0) {
  console.error(`Step 12 Finance Intelligence acceptance failed with exit ${result.code ?? result.signal} after ${attempts} attempt(s).`)
