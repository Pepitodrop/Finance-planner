...
test('normalizes routes without exposing identifiers or query strings', () => {
  ...
  assert.equal(operationalRoute('/api/connectors/licensed-aisp/start?state=secret'), '/api/connectors/:provider/start')
  assert.equal(operationalRoute('/api/connectors/webhooks/paypal'), '/api/connectors/webhooks/:provider')
  assert.equal(operationalRoute('/api/connectors/webhooks/licensed-aisp'), '/api/connectors/webhooks/:provider')
...
test('records bounded HTTP, bank, AI and deletion metrics', () => {
  ...
  metrics.recordBank('licensed-aisp', 'success')
  metrics.recordBank('INVALID PROVIDER VALUE', 'failure')
  ...
  assert.match(output, /finance_planner_bank_operations_total\{outcome="success",provider="licensed-aisp"\} 1/)
  assert.match(output, /finance_planner_bank_operations_total\{outcome="failure",provider="other"\} 1/)
