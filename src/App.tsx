...
import { ArrowDownRight, ArrowUpRight, BrainCircuit, CalendarClock, Landmark, MessageCircleQuestion, PiggyBank, Plus, Repeat2, Target, WalletCards } from 'lucide-react'
...
type Tab = 'dashboard' | 'transactions' | 'goals' | 'recurring' | 'ai' | 'assistant'
...
function App() {
  ...
  const addTransaction = (formData: FormData) => {
    ...
    const transaction = { id: crypto.randomUUID(), accountId: String(formData.get('accountId')), description: String(formData.get('description')), category: String(formData.get('category')), type: transactionType, amountCents: Math.round(amount * 100), date: String(formData.get('date')), recurring: formData.get('recurring') === 'on' }
    setState((current) => ({ ...current, transactions: [transaction, ...current.transactions], accounts: current.accounts.map((account) => account.id === transaction.accountId ? { ...account, balanceCents: account.balanceCents + (transaction.type === 'income' ? transaction.amountCents : -transaction.amountCents) } : account) }))
  ...
  const applyAiSuggestion = (transactionId: string, suggestion: AiSuggestion) => {
    ...
    setState((current) => ({ ...current, transactions: current.transactions.map((transaction) => transaction.id === transactionId ? { ...transaction, description: suggestion.merchant, category: suggestion.category, recurring: suggestion.recurringProbability >= 75 ? true : transaction.recurring } : transaction) }))
  }

  const title = tab === 'dashboard' ? 'Finanzübersicht' : tab === 'transactions' ? 'Transaktionen' : tab === 'goals' ? 'Sparziele' : tab === 'recurring' ? 'Wiederkehrende Zahlungen' : tab === 'ai' ? 'KI-Kategorisierung' : 'Finanzanalyse & Planung'
