...
export function FinanceAssistant({ state }: { state: AppState }) {
  ...
  const run = async () => {
    ...
    try {
      const result = await runAssistant(mode, state, question.trim())
      setAnswer(result)
  ...
  return <div className="assistant-page">
    <section className="panel assistant-hero">
      <div><p className="eyebrow">Gehostete, kontrollierte KI</p><h2>Analyse, Fragen und Finanzplanung</h2><p>Nur eine kompakte, aggregierte Zusammenfassung wird an das Backend gesendet. Geldberechnungen bleiben deterministisch.</p></div>
      <div className="ai-model"><BrainCircuit size={18}/><div><strong>{ASSISTANT_MODEL}</strong><span>Serverseitig · kein großer Browser-Download</span></div></div>
    </section>
    ...
    <section className="assistant-grid">
      <article className="panel">
        ...
        <button className="primary" onClick={run} disabled={loading || !question.trim()}>{loading ? <LoaderCircle className="spin" size={18}/> : <Send size={18}/>} {loading ? 'Analyse läuft …' : 'Assistent starten'}</button>
        {loading && <div className="status-message" role="status"><LoaderCircle className="spin" size={18}/><div><strong>Deine Finanzanalyse wird erstellt</strong><span>Daten werden zusammengefasst, Modelle abgeglichen und die Antwort vorbereitet. Das kann bis zu einer Minute dauern.</span>{startedAt && <small>Anfrage gestartet um {new Date(startedAt).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</small>}</div></div>}
      </article>
