...
export function TransactionsPage({ transactions, accounts, onEdit, onDelete }: TransactionsPageProps) {
  ...
  const resetFilters = () => {
    setTypeFilter('all'); setSearchQuery(''); setCategoryFilter('all'); setAccountFilter('all'); setAmountFilter('all'); setDateFilter('all'); setSelectedIds(new Set())
  }
  ...
  return <section className="transactions-reference-layout" aria-label="Transaktionsverwaltung">
    <article className="panel transactions-reference-main">
      <div className="transactions-toolbar">
        <div className="transaction-tabs" role="group" aria-label="Transaktionsart">
          ...
          <button type="button" className={typeFilter === 'transfer' ? 'active' : ''} aria-pressed={typeFilter === 'transfer'} onClick={() => setTypeFilter('transfer')}>Transfers</button>
        </div>
        <div className="transactions-toolbar-actions">
          ...
          <button type="button" className="transactions-filter-trigger"><Filter size={16}/> Filter</button>
    ...
    <aside className="transactions-reference-sidebar">
      <article className="panel transactions-summary-card"><div className="panel-header"><h2>Zusammenfassung</h2><span className="pill">Dieser Monat</span></div><dl><div><dt>Einnahmen</dt><dd className="positive-text">{formatMoney(totals.incomeCents)}</dd></div><div><dt>Ausgaben</dt><dd className="negative-text">-{formatMoney(totals.expenseCents)}</dd></div><div className="summary-net"><dt>Netto</dt><dd>{formatMoney(totals.incomeCents - totals.expenseCents)}</dd></div></dl></article>

      <article className="panel transactions-category-card"><div className="panel-header"><h2>Ausgaben nach Kategorie</h2><span className="pill">Dieser Monat</span></div><div className="transactions-category-chart"><ResponsiveContainer width="100%" height={220}><PieChart><Pie data={categories} dataKey="value" nameKey="name" innerRadius={58} outerRadius={88} paddingAngle={2}>{categories.map((category, index) => <Cell key={category.name} fill={CATEGORY_COLORS[index % CATEGORY_COLORS.length]}/>)}</Pie><Tooltip formatter={(value) => formatMoney(Number(value) * 100)}/></PieChart></ResponsiveContainer><div className="transactions-category-total"><strong>{formatMoney(totalCategoryValue * 100)}</strong><span>Gesamt</span></div></div><div className="transactions-category-legend">{categories.slice(0, 6).map((category, index) => { const percentage = totalCategoryValue > 0 ? Math.round((category.value / totalCategoryValue) * 100) : 0; return <div key={category.name}><span><i style={{ background: CATEGORY_COLORS[index % CATEGORY_COLORS.length] }}/>{category.name}</span><span>{percentage}%</span><strong>{formatMoney(category.value * 100)}</strong></div> })}</div></article>

      <article className="panel transactions-filter-card"><div className="panel-header"><h2>Filter</h2><button type="button" className="panel-link" onClick={resetFilters}>Alle löschen</button></div><label>Zeitraum<div className="filter-select-wrap"><CalendarDays size={15}/><select value={dateFilter} onChange={(event) => setDateFilter(event.target.value)}><option value="all">Alle Zeiträume</option><option value="month">Dieser Monat</option><option value="30days">Letzte 30 Tage</option></select></div></label><label>Kategorie<select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="all">Alle Kategorien</option>{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label>Konto<select value={accountFilter} onChange={(event) => setAccountFilter(event.target.value)}><option value="all">Alle Konten</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label>Betrag<select value={amountFilter} onChange={(event) => setAmountFilter(event.target.value)}><option value="all">Alle Beträge</option><option value="small">Unter 50 €</option><option value="medium">50–500 €</option><option value="large">Ab 500 €</option></select></label></article>
    </aside>
  </section>
}
