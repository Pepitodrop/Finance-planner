...
export interface SyncPreview { accountsToCreate: Account[]; transactionsToImport: Transaction[]; duplicateCount: number; pendingCount: number }
...
export function buildSyncPreview(state: AppState, payload: SyncPayload): SyncPreview {
  ...
  const known = new Set(state.transactions.map(transactionFingerprint)); const transactionsToImport: Transaction[] = []; let duplicateCount = 0; let pendingCount = 0
  for (const external of payload.transactions) {
    ...
    const transaction: Transaction = { id: `connector:${payload.connection.provider}:${external.externalId}`, accountId, description: normalizeDescription(external.description), category: external.category?.trim() || 'Unkategorisiert', type: external.amountCents >= 0 ? 'income' : 'expense', amountCents: Math.abs(external.amountCents), date: external.bookingDate, recurring: false }
