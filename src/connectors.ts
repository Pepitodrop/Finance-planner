...
function normalizeDescription(value: string): string {
  return value.replace(/\s+/g, ' ').trim().slice(0, 160) || 'Unbenannte Transaktion'
}

export function transactionFingerprint(transaction: Pick<Transaction, 'accountId' | 'date' | 'amountCents' | 'description'>): string {
  return [
    transaction.accountId,
    transaction.date,
    transaction.amountCents,
    normalizeDescription(transaction.description).toLocaleLowerCase('de-DE'),
  ].join('|')
}
...
export function buildSyncPreview(state: AppState, payload: SyncPayload): SyncPreview {
  ...
  for (const external of payload.accounts) {
    ...
    if (!existing) accountsToCreate.push({
      id: deterministicId,
      name: external.name,
      type: external.type,
  ...
  const known = new Set(state.transactions.map(transactionFingerprint))
  ...
  for (const external of payload.transactions) {
    ...
    const accountId = accountMap.get(external.externalAccountId)
    if (!accountId) continue
    const transaction: Transaction = {
      id: `connector:${payload.connection.provider}:${external.externalId}`,
      accountId,
      description: normalizeDescription(external.description),
      category: external.category?.trim() || 'Unkategorisiert',
      type: external.amountCents >= 0 ? 'income' : 'expense',
      amountCents: Math.abs(external.amountCents),
      date: external.bookingDate,
      recurring: false,
    }
    const fingerprint = transactionFingerprint(transaction)
    if (known.has(fingerprint) || state.transactions.some((item) => item.id === transaction.id)) {
      duplicateCount += 1
      continue
    }
  ...
  return { accountsToCreate, transactionsToImport, duplicateCount, pendingCount }
}

export function applySyncPreview(state: AppState, preview: SyncPreview): AppState {
  return {
    ...state,
    accounts: [...state.accounts, ...preview.accountsToCreate],
...
export async function startConnector(provider: ConnectorProvider, backendBaseUrl: string): Promise<void> {
  const response = await fetch(`${backendBaseUrl.replace(/\/$/, '')}/api/connectors/${provider}/start`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({ redirectUri: window.location.href, country: 'DE' }),
  })
...
export async function synchronizeConnections(backendBaseUrl: string): Promise<SyncPayload[]> {
  const response = await fetch(`${backendBaseUrl.replace(/\/$/, '')}/api/connectors/sync`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
  })
