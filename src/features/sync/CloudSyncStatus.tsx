...
import { AlertTriangle, CheckCircle2, CloudOff, Database, LoaderCircle, RefreshCw } from 'lucide-react'
...
export function CloudSyncStatus() {
  ...
  const Icon = status.phase === 'synced' ? CheckCircle2
    : status.phase === 'syncing' ? LoaderCircle
  ...
  return <aside className={`cloud-sync-status ${status.phase}`} role={status.phase === 'conflict' || status.phase === 'error' ? 'alert' : 'status'} aria-live="polite">
    ...
    <div><strong>{status.phase === 'synced' ? 'Cloud gespeichert' : status.phase === 'syncing' ? 'Synchronisierung' : status.phase === 'conflict' ? 'Datenkonflikt' : status.phase === 'offline' ? 'Lokaler Modus' : status.phase === 'error' ? 'Speicherfehler' : 'Lokaler Speicher'}</strong><span>{status.message}</span></div>
