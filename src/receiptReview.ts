...
function imageFromObjectUrl(file: File): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => { URL.revokeObjectURL(objectUrl); reject(new Error('The image could not be read.')) }
    image.src = objectUrl
  })
}
...
function canvasToBlob(canvas: HTMLCanvasElement, quality: number): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('The receipt could not be compressed.')), 'image/jpeg', quality)
  })
}
...
function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => typeof reader.result === 'string' ? resolve(reader.result) : reject(new Error('The receipt could not be prepared.'))
    reader.onerror = () => reject(new Error('The receipt could not be prepared.'))
    reader.readAsDataURL(blob)
  })
}

export async function prepareReceiptImage(file: File): Promise<PreparedReceiptImage> {
  if (!SUPPORTED_RECEIPT_TYPES.has(file.type)) throw new Error('Please choose a JPEG, PNG, or WebP image.')
  if (!file.size || file.size > MAX_SOURCE_BYTES) throw new Error('The source image must be at most 10 MB.')
  const image = await imageFromObjectUrl(file)
  if (!image.naturalWidth || !image.naturalHeight) throw new Error('The image does not have valid dimensions.')
  ...
  if (!compressed || compressed.size > MAX_COMPRESSED_BYTES) throw new Error('The image is still too large after compression. Try photographing it closer or cropping it.')
...
function integerScore(value: unknown, field: string): number {
  if (!Number.isInteger(value) || Number(value) < 0 || Number(value) > 100) throw new Error(`Invalid value for ${field}.`)
  return Number(value)
}

function confidence(value: unknown): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) throw new Error('Invalid model confidence.')
  return value
}
...
export function validateReceiptReviewResponse(value: unknown): ReceiptReviewResult {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('The receipt analysis returned an invalid result.')
  const result = value as Partial<ReceiptReviewResult>
  if (result.currency !== 'EUR' || result.imageStored !== false || !result.model || typeof result.model !== 'object') throw new Error('The receipt analysis is incomplete.')
  if (result.evidenceStatus !== 'sufficient' && result.evidenceStatus !== 'insufficient') throw new Error('The evidence status of the receipt analysis is invalid.')
  if (!Array.isArray(result.items) || !Array.isArray(result.recommendations) || !Array.isArray(result.limitations)) throw new Error('The receipt analysis contains invalid lists.')

  const items = result.items.map((item, index) => {
    if (!item || typeof item !== 'object' || typeof item.name !== 'string' || typeof item.assessment !== 'string' || !Array.isArray(item.alternativeStores)) throw new Error(`Item ${index + 1} is invalid.`)
    if (!item.labels || typeof item.labels !== 'object') throw new Error(`Labels for item ${index + 1} are invalid.`)
    return item
  })
  ...
  if (result.evidenceStatus === 'sufficient') {
    score = integerScore(result.score, 'overall score')
    subScores = {
      affordability: integerScore(result.subScores?.affordability, 'price'),
      bioFairTrade: integerScore(result.subScores?.bioFairTrade, 'bio and fair trade'),
      eco: integerScore(result.subScores?.eco, 'environment'),
    }
  } else {
    if (result.score !== null || result.subScores !== null || items.length > 0 || result.recommendations.length > 0) {
      throw new Error('An insufficient-evidence receipt analysis must not include a score or recommendations.')
    }
    score = null
    subScores = null
  }
...
export async function requestReceiptReview(image: PreparedReceiptImage, signal?: AbortSignal): Promise<ReceiptReviewResult> {
  ...
  if (!response.ok) {
    ...
    throw new Error(typeof message === 'string' ? message : 'The receipt analysis failed.')
  }
  return validateReceiptReviewResponse(payload)
}

export function formatReceiptMoney(cents: number | null): string {
  return cents === null ? 'not detected' : (cents / 100).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })
}
